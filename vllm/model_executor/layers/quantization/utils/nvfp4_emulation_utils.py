...
# True when the device supports native FP8 (CUDA sm_90+, ROCm gfx942+).
USE_NATIVE_FP8 = current_platform.supports_fp8()
...
def float8_qdq_no_fp8(scale: torch.Tensor) -> torch.Tensor:
    ...
    fp8_scale = amax_to_scale(
        torch.max(torch.abs(scale_clamped)), e4m3_type, scale.dtype
    )
...
def ref_nvfp4_quant(x: torch.Tensor, global_scale: torch.Tensor, block_size: int):
    ...
    scale = float8_qdq_native(scale) if USE_NATIVE_FP8 else float8_qdq_no_fp8(scale)
