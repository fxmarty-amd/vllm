...
"""
Fused NVFP4 emulation for MoE.

Unlike nvfp4_emulation_moe.py which runs weight dequantization, activation QDQ
and BF16 GEMM as three separate kernel launches, this implementation fuses
weight dequantization and GEMM into a single Triton kernel
(fused_moe_nvfp4_emulation_kernel).

Activation QDQ is kept separate (via moe_kernel_quantize_input) since the
activation tensor is small compared to the weight tensor, and running QDQ
once avoids redundant recomputation across top-k experts.

Inside the kernel's K-loop:
  1. Load BF16 activations (already QDQ'd).
  2. Load packed uint8 NVFP4 weights, dequantize (E2M1 decode + scale)
     in-register.
  3. tl.dot with FP32 accumulation.

This avoids materializing the full dequantized BF16 weight tensor in global
memory.
"""
...
# ──────────────────────────────────────────────────────────────────────
# Fused kernel: weight dequant + GEMM
# ──────────────────────────────────────────────────────────────────────
...
class FusedNvfp4EmulationTritonExperts(TritonExperts):
    """
    Extension of TritonExperts for emulated NVFP4 MoE with fused kernels.

    Activation QDQ is applied separately via moe_kernel_quantize_input.
    Weight dequantization and GEMM are fused into a single Triton kernel.
    """
    ...
    def __init__(
        ...
        logger.warning_once(
            "Using FusedNvfp4EmulationTritonExperts MOE backend. This"
            " emulates NVFP4 quantization and may be slower than native"
            " NVFP4 on supported hardware (e.g. Nvidia Blackwell)."
        )
    ...
    def apply(
        ...
        # ── Activation QDQ on hidden_states (a1) ──
        ...
        # ── W13 GEMM: fused weight dequant + GEMM ──
        ...
        # ── Activation (e.g. SiLU + Mul) between w13 and w2 ──
        self.activation(
            activation, intermediate_cache2, intermediate_cache1.view(-1, N)
        )

        # ── Activation QDQ on intermediate_cache2 (a2) ──
        ...
        # ── W2 GEMM: fused weight dequant + GEMM ──
        invoke_fused_moe_nvfp4_emulation_kernel(
